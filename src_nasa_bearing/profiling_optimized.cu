...
int main() {
    ...
    std::cout << "=== OPTIMIZED SCENARIO 1: Concurrent Streams ===" << std::endl;
    std::cout << "Showing improved efficiency with async operations\n" << std::endl;
    ...
    float avg_ms = total_ms / NUM_FILES;
    
    std::cout << "=== OPTIMIZED RESULTS (RTX A4000) ===" << std::endl;
    printf("Total time per file: %.4f ms\n", avg_ms);
    printf("Speedup vs Naive (0.1409 ms): %.2fx\n", 0.1409 / avg_ms);
    
    std::cout << "\n✅ Improvements:" << std::endl;
    std::cout << "   • Async transfers (cudaMemcpyAsync)" << std::endl;
    std::cout << "   • Pinned memory (cudaMallocHost)" << std::endl;
    std::cout << "   • 4 concurrent streams (multi-bearing)" << std::endl;
    std::cout << "   • Overlapped execution (transfers + kernels)" << std::endl;
